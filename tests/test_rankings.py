...
class TestRealEventRanking:
    def _make_competitors(self, edition, enabled_category, n):
        from apps.participants.models import Person
    ...
    def test_event_ranking_asc_time(self, event, status_valid, edition, enabled_category):
        competitors = self._make_competitors(edition, enabled_category, 4)
    ...
    def test_dense_ranking_ties(self, event, status_valid, edition, enabled_category, scoring_rules):
        competitors = self._make_competitors(edition, enabled_category, 4)
    ...
    def test_disqualified_not_ranked(self, event, status_valid, status_disqualified, edition, enabled_category):
        competitors = self._make_competitors(edition, enabled_category, 2)
...
class TestCompetitionRankingService:
    ...
    def test_final_score_sum(self, event, status_valid, edition, enabled_category, scoring_rules):
        competitors = self._make_competitors(edition, enabled_category, 2)
        ...
        ranking = service.calculate_competition_ranking(edition, event.competition_stage)
    ...
    def test_tie_breaking_places(self, event, status_valid, edition, enabled_category, scoring_rules):
        competitors = self._make_competitors(edition, enabled_category, 2)

        # Both get 488, but competitor 1 has 3 wins (1st place finishes) vs competitor 2 with 2 wins
        events = [event]
        for i in range(2, 6):
            events.append(Event.objects.create(
                competition_stage=event.competition_stage,
        ...
        from apps.participants.models import Person
        people = [
            Person.objects.create(first_name='X1', last_name='Lopez', gender='M'),
            Person.objects.create(first_name='X2', last_name='Lopez', gender='M'),
            Person.objects.create(first_name='X3', last_name='Lopez', gender='M'),
        ]
        comps = []
        ...
        entry = ranking[list(ranking.keys())[0]]
        # Competitor 1 has 2 wins (1st places) vs competitor 2 also has 2 wins → rank tie... 
        # Actually C1: WOD1=1st, WOD2=1st (2 wins); C2: WOD3=1st, WOD4=1st (2 wins)
        # Tie-breaker: more 1st places → tie → more 2nd places → both have 2 → maintain tie
        # So rank of both should be 1
...
class TestFinalQualificationService:
    def test_qualifier_top_n(self, event, status_valid, edition, enabled_category, scoring_rules):
        from apps.participants.models import Person

        competitors = []
        for i in range(8):
            p = Person.objects.create(first_name=f'A{i}', last_name='Lopez', gender='M')
            c = Competitor.objects.create(
                competition_edition=edition,
                competitor_type=Competitor.CompetitorType.INDIVIDUAL,
                person=p,
        ...
        times = [f'0{i + 1}:00' if i + 1 < 10 else f'10:00' for i in range(8)]
        ...
        qualifiers = service.get_qualifiers(edition)
